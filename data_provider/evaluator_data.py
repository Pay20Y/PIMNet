...
class Evaluator(object):
    def __init__(self, lmdb_data_dir, batch_size, height, width, max_len, keep_ratio, voc_type='LOWERCASE'):
        ...
        self.voc, self.char2id, self.id2char = get_vocabulary(self.voc_type)
        print("vocabulary: ", len(self.voc))
        self.index = 1
    ...
    def get_batch(self):
        ...
        while len(batch_images) < self.batch_size:
            try:
                ...
                if self.keep_ratio:
                    new_width = int((1.0 * self.height / H) * W)
                ...
                label = np.full((self.max_len), self.char2id['EOS'], dtype=np.int)
        ...
        return np.array(batch_images), np.array(batch_labels), np.array(batch_masks), np.array(batch_lengths), batch_labels_str